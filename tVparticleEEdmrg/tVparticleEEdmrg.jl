# Entanglement entropy of the t-V Model at half-filling - dependence on interaction strength V
...
function parse_commandline()
    ...
    add_arg_group(s, "dmrg parameters")
    @add_arg_table s begin
        "--seed"
        metavar = "seed"
    ...
    @add_arg_table s begin
        "--ee"
        metavar = "n"
        help = "compute all EEs with partition size n"
        arg_type = Int
        required = true
    end
...
function main()
    ...
    if c[:L] != 2 * c[:N]
        print("Not at half-filling: got number of sites L=", c[:L], " and got number of particles N=", c[:N], " exit(1)")
        exit(1)
    end 
